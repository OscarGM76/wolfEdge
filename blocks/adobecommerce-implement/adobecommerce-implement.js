...
export default function decorate(block) {
  try {
    const arraySectionHead = document.querySelectorAll(
      "div.adobecommerce-implement-container > div > div"
    );
    const arraySectionTab = document.querySelectorAll(
      "div.tab-option-container > div > div"
    );
    const arraySectionCover = document.querySelectorAll(
      "div.adobecommerce-cover-container > div > div"
    );
    ...
    if (arraySectionCover?.length > 0) {
      addClass(arraySectionCover);
    }
